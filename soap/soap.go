...
// CallContextWithFaultDetail CallContextWithFault performs HTTP POST request.
// Note that if SOAP fault is returned, it will be stored in the error.
...
func (s *Client) callAuth(ctx context.Context, soapAction string, request, response interface{}, faultDetail FaultError,
	...
	if res.StatusCode == 401 {
		return s.callSecurity(ctx, soapAction, request, response, faultDetail, retAttachments)
	}
...
func (s *Client) callSecurity(ctx context.Context, soapAction string, request, response interface{}, faultDetail FaultError,
	retAttachments *[]MIMEMultipartAttachment) error {
	// SOAP envelope capable of namespace prefixes
	envelope := SOAPEnvelope{}
